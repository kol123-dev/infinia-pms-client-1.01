...
export default function TenantsPage() {
  ...
  return (
    <MainLayout>
      ...
      {isFormOpen && isEditing && selectedTenant !== undefined && (
        <TenantEditDialog
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      fd.append('create_users', String(createUsers))
      const res = await api.post('/tenants/import/', fd, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
      setImportResult(res.data)
      toast({ description: `Import done: ${res.data.created} created, ${res.data.updated} updated.` })
      // Refresh table
      const response = await api.get(`/tenants/?page=${pageIndex + 1}`)
      setTenants(response.data.results)
    } catch (error: any) {
