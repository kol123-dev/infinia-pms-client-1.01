...
import { Building, Phone, Mail, Plus, Eye, MessageSquare } from "lucide-react"
...
export default function Landlords() {
  ...
  const handleAddLandlord = async (data: any) => {
    try {
      ...
      toast({ description: "Landlord created successfully" })
      setIsFormOpen(false)
  ...
  const handleEditLandlord = async (data: any) => {
    try {
      ...
      toast({ description: "Landlord updated successfully" })
      setIsFormOpen(false)
      // Refresh the landlords list
      const response = await api.get('/landlords/')
      setLandlords(response.data.results)
  ...
  const handleDeleteLandlord = async () => {
    try {
      ...
      toast({ description: "Landlord deleted successfully" })
    ...
    <MainLayout>
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        <Card className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-950 dark:to-blue-900">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{landlords.length}</div>
      ...
      <LandlordForm
        ...
        onClose={() => {
          setIsFormOpen(false)
          setIsEditing(false)
          setSelectedLandlord(undefined)  // Instead of null
        }}
